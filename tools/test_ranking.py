...
with open('data/test_real.json', 'r', encoding='utf-8') as f:
    rivs = json.load(f)
...
for r in rivs:
    ...
    for k in CONTEXT_WEIGHTS:
        val = mc.get(k, {}).get('value', 0)
        if val:
            w_tot += CONTEXT_WEIGHTS[k] * val
    r['weighted_total'] = w_tot

ncap_weighted = 412.0 # usare l'ncap della v2 generale per comparabilit
...
print(f"Snapshot V2 su dati verificati (N_cap: {ncap_weighted:.1f}):")
